...
def EC_3D(stretch, param):
    ...
    return param[0]/(J*lamChain) * invLangevin(lamChain/param[1]) / invLangevin(1/param[1]) * devbstar + param[2]*(J-1) * eye(3)

def toVec(A):
    return array([A[0][0], A[1][1], A[2][2]], dtype='float64')

def uniaxial_stress_visco(model, timeVec, trueStrainVec, params, aT):
